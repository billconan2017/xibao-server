...
router.get('/client/config', requireAuth, (req, res) => {
  res.json({
    ...
    data: {
      ...
      version: getSetting('client_version', '2.2.0'),
...
router.get('/update', (req, res) => {
  res.json({
    code: 0,
    data: {
      version: getSetting('version', '2.2.0'),
      latest: getSetting('version', '2.2.0'),
      has_update: false,
    },
  });
});
...
router.get('/about', (req, res) => {
  const changelog = [
    { version: 'v2.2.0', date: '2026-09-16', desc: '家庭管理界面、频道编辑与排序、本地 TXT 分组导入；修复客户端启动闪退，增加自动播放、收藏和重连' },
  ...
  res.json({ code: 0, data: { changelog, version: getSetting('version', '2.2.0') } });
});
